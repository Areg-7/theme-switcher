...
import {
	...
	Menu,
	MenuItem,
} from '@/components/ui/ThemeSwitcherUI'
import { changeTheme, customTheme, navLinks } from '@/data'
import { cn } from '@/lib/utils'
import { useState } from 'react'

export function ThemeSwitcher() {
	return (
		<header className='padding-x py-8 absolute z-50 w-full text-text'>
			<nav className='flex justify-between items-center max-container'>
				<svg
					...
					className='m-0 w-[129px] h-[29px]  fill-text-primary'
				>
					<path
				...
				<Navbar className='top-2 text-lg font-medium font-montserrat' />
		...
		>
			<Menu setActive={setActive}>
				<MenuItem setActive={setActive} active={active} item='Menu'>
					<div className='flex flex-col space-y-4 text-sm'>
						{navLinks.map((item: { label: string; href: string }) => (
							<HoveredLink href={item.href} key={item.label}>
								{item.label}
							</HoveredLink>
						))}
					</div>
				...
				<MenuItem setActive={setActive} active={active} item='Custom'>
					<div className='flex flex-col space-y-4 text-sm w-50 text-center'>
						{customTheme.map((item: { color: string; title: string }) => (
							<>
